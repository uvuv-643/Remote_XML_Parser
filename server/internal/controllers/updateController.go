...
func serverUnavailableUpdate(c *gin.Context, err error, config *services.Config, signal chan<- struct{}) {
...
func UpdateHandler(c *gin.Context, config *services.Config, signal chan<- struct{}) {
	...
	if status == global.Updating {
		serverUnavailableUpdate(c, errors.New("already updating"), config, signal)
		return
	}
	...
	if err != nil {
		...
		if len(itemIdsInDatabase) == 0 {
			if err := config.DBClient.Model(&dbs.SDNItem{}).Pluck("uid", &itemIdsInDatabase).Error; err != nil {
				serverUnavailableUpdate(c, errors.New("cannot parse removed elements"), config, signal)
			}
		}
		for _, itemId := range itemIdsInDatabase {
			if !sdnItemsIdsSet[itemId] {
				...
				if err := config.DBClient.Delete(&dbs.SDNItem{}, itemId).Error; err != nil {
					serverUnavailableUpdate(c, errors.New("cannot delete element from database"), config, signal)
				}
			}
		}
		...
		if err != nil {
			serverUnavailableUpdate(c, errors.New(""), config, signal)
			return
		}
			...
			if err := config.DBClient.First(&dbs.SDNProgram{}, "program = ?", program.Program).Updates(program).Error; err != nil {
				if errors.Is(err, gorm.ErrRecordNotFound) {
					if err = config.DBClient.Create(&program).Error; err != nil {
						serverUnavailableUpdate(c, err, config, signal)
						return
					}
				}
			}
